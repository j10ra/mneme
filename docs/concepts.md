...
| **Shadow** | Legacy concept. `memories.chunk_id UNIQUE` makes identical-content rows impossible by construction, so `meta.shadow_of` is no longer produced. The filter still lives in recall as defensive code. |
