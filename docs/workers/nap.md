...
2. **Recall-weight decay (LTP).** Multiplies `recall_weight` by `RECALL_LTD_DECAY = 0.933` per cycle so use-driven reinforcement fades when unused. With 6 cycles/day this preserves a ~40 hour half-life from a single recall hit. Decay stops once `recall_weight < 0.01` (the UPDATE only touches rows at or above the floor). Same keyset pagination as importance decay.
