...
| Extraction LLM | `local` provider → any OpenAI-compatible chat-completions endpoint with streaming + JSON output (e.g. Ollama serving `qwen2.5:3b-instruct-q4_K_M`) | Self-hosted (Ollama, vLLM, llama.cpp); remote APIs (Groq, OpenRouter, Anthropic, OpenAI, Mistral) added under `llm/` | Streaming responses required so slow generations don't trip 100s+ idle timeouts at the edge. JSON output via `response_format: { type: "json_object" }`. 90s client-side timeout. Circuit breaker pauses the worker for 5 min after 3 consecutive failures. |
...
**Coalescing rule:** captures with the same `session_id` arriving within a ±5-minute window of the seed are extracted in one LLM call (capped at 10 captures, 1500 chars each, 3000 chars total). One LLM call returns N observations, so the typical coalesced batch produces several `memories` rows from one inference. This is the cost lever vs. per-event extraction; combined with composite `chunk_id` it also makes retries idempotent.

**Provider-agnostic by construction.** Both workers call into `extractObservations()` / `embedBatch()` from `packages/server/src/llm/index.ts` and `embedder/index.ts`. Those modules pick the concrete provider via `LLM_PROVIDER` / `EMBEDDER_PROVIDER` env vars at startup. Every concrete provider speaks an OpenAI-compatible (LLM) or TEI-compatible (embeddings) shape, so swapping is one env var + one `index.ts` map entry.

**Failure modes are bounded.** On extract failure: per-job exponential backoff (`attempts * 2 min`) up to 5 attempts, then nap escalates transient vs. dead (§6.3.1). After 3 consecutive cycle failures the worker opens a 5-minute circuit breaker so a downed LLM doesn't burn job attempts. On embed failure: same backoff with a 30-second base, no circuit breaker (the embedder is faster and less likely to flap).
