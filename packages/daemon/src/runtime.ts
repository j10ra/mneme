...
//
// Coalescing (group by session_id + repo + private within ±5min) is
// deferred. Phase 1 processes one capture per LLM call. Coalescing is a
// measurable optimization to layer on once end-to-end is healthy.
...
export function createRuntime(deps: DaemonDeps) {
  ...
  async function runWorkerTick(): Promise<void> {
    // Stage 1: pending -> extracted
    await processStage("pending", async (id, data) => {
      const capture = data as CaptureBody;
      const memories = await deps.extract([capture]);
      await deps.outbox.transition(id, "pending", "extracted", {
        capture,
        memories,
      });
    });
